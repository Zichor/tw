...
for (var i = 0; i < localVillages.length; i++) {
    if (localVillages[i].newbie.toString().indexOf("tomorrow") == 0) {
        localVillagesDateOut.push(new Date(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate() + 1, localVillages[i].newbie.match(/([0-9]{2}):([0-9]{2})/)[1], localVillages[i].newbie.match(/([0-9]{2}):([0-9]{2})/)[2], "00", "000"));
    }
    if (localVillages[i].newbie.toString().indexOf("today") == 0) {
        console.log("today");
        localVillagesDateOut.push(new Date(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate(), localVillages[i].newbie.match(/([0-9]{2}):([0-9]{2})/)[1], localVillages[i].newbie.match(/([0-9]{2}):([0-9]{2})/)[2], "00", "000"));
    }
    ...
    if (localVillages[i].newbie != 0 && localVillages[i].newbie.indexOf("tomorrow") < 0 && localVillages[i].newbie.indexOf("today") < 0) {
        ...
        localVillagesDateOut.push(new Date(today.getUTCFullYear(), parseInt(localVillages[i].newbie.match(/([0-9]{2}).([0-9]{2})/)[1] - 1), localVillages[i].newbie.match(/([0-9]{2}).([0-9]{2})/)[2], localVillages[i].newbie.match(/([0-9]{2}):([0-9]{2})/)[1], localVillages[i].newbie.match(/([0-9]{2}):([0-9]{2})/)[2], "00", "000"));
    }
}
...
for (var i = 0; i < localVillages.length; i++) {
    ...
    if (ignore == true) {
        ...
        if (localVillagesDateOut[i] != "out of BP") {
            //create date
            if (localVillagesDateOut[i].getUTCHours() + 2 > 23) {
                console.log("would have been over 24")
                tempDate = parseInt(localVillagesDateOut[i].getUTCDate() + 1) + "/" + (localVillagesDateOut[i].getUTCMonth() + 1) + "/" + new Date().getUTCFullYear() + " ";
            }
            else {
                tempDate = localVillagesDateOut[i].getUTCDate() + "/" + (localVillagesDateOut[i].getUTCMonth() + 1) + "/" + new Date().getUTCFullYear() + " ";
            }
            ...
            if (localVillagesDateOut[i].getUTCHours() + 2 < 10) {
                ...
                if (localVillagesDateOut[i].getUTCHours() + 2 > 23) {
                    tempDate += "0" + parseInt(localVillagesDateOut[i].getUTCHours() + 2)-24;
                }
                else {
